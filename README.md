...
- **Automatic (recommended)**: generates a key + CSR locally and calls the Cloudflare API. Needs a one-time **Origin CA Key** from your Cloudflare account (dashboard → *My Profile → API Tokens → Origin CA Key*) — the same key is reused for every future domain, the script only asks for it once per run (export `CF_ORIGIN_CA_KEY` to skip the prompt entirely on future runs).
